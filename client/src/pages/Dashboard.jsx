...
function Dashboard() {
  ...
  const uploadResume = async (e) => {
    ...
    try {
      ...
      const resumeText = await (pdfToText.default ? pdfToText.default(resume) : pdfToText(resume));
      ...
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          {allResumes.map((resume, index) => {
            ...
            return (
              <button key={index} onClick={() => navigate(`/app/builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-1g gap-2 border group hover:shadow-1g transition-all duration-300 cursor-pointer'
