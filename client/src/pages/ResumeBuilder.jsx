...
import { ArrowLeftIcon, Briefcase, ChevronLeft, ChevronRight, DownloadIcon, EyeIcon, EyeOffIcon, FileSearch, FileText, FolderIcon, GraduationCap, Share2Icon, Sparkles, User } from 'lucide-react'
...
function ResumeBuilder() {
  ...
  const sections = [
    ...
    { id: "jobDescription", name: "Job Description", icon: FileSearch },
  ]
  ...
  const saveResume = async () => {
    ...
    try {
      ...
      const newData = JSON.stringify(data.resume);
      if (newData !== currentData) {
        lastSavedData.current = newData;
        setResumeData(data.resume);
      }

      // ❗ optional: avoid toast spam
      // toast.success(data.message);
            ...
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1'>
              ...
              <div className="flex justify-between items-center mb-6 border-b border-gray-300 py-1">
                ...
                <div className='flex items-center'>
                  ...
                  <button onClick={() => setActiveSectionIndex((prevIndex) => Math.min(prevIndex + 1, sections.length - 1))} className={`flex items-center gap-1 p-3 rounded-1g text-sm font-medium text-gray-600 hover: bg-gray-50 transition-all ${activeSectionIndex === sections.length - 1 && 'opacity-50'}`} disabled={activeSectionIndex === sections.length - 1}>
              ...
              <div className='space-y-6'>
                ...
                {activeSection.id === 'experience' && (
                  <ExperienceForm generatingIndex={generatingIndex} setGeneratingIndex={setGeneratingIndex} setDisabled={setDisabled} disabled={disabled} data={resumeData.experience} onChange={(data) => setResumeData(prev => ({ ...prev, experience: data }))} />
                )}
          ...
          <div className='lg:col-span-7 max-lg:mt-6'>
            <div className='relative w-full'>
              <div className='absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2'>
                {resumeData.public && (
                  <button onClick={handleShare} className='flex items-center p-2 px-4 gap-2 text-xs bg-gradient-to-br from-blue-100 to-blue-200 text-blue-600 rounded-1g ring-blue-300 hover:ring transition-colors'>
